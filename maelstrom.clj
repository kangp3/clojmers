...
(defn parse-msg
  "Parses an incoming message, turning its string keys at the msg and body
  level into keywords. Leaves inner structures alone."
  [msg]
  (-> msg
      (update-keys keyword)
      (update :body update-keys keyword)))
...
(defn process-stdin!
  ...
  (doseq [line (line-seq (BufferedReader. *in*))]
    (future
      (let [req (-> line json/parse-string parse-msg)]
        (try
